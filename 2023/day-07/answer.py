...
def get_hand(hand):
    ...
    if len([x for x in counts if x == 3]) > 0 and len([x for x in counts if x == 2]) > 0:
...
def part_a(filename):
    ...
    with open(filename) as f:
        ...
        w = 0
        sortedHands = sorted(hands.keys(), key=cmp_to_key(cmp_hand))
...
def cmp_hand_with_jokers(hand1: HandWithJoker, hand2: HandWithJoker):
    v1 = get_hand(hand1.bestHand)
    v2 = get_hand(hand2.bestHand)

    if v1 == v2:
        for a, b in zip(hand1.origHand, hand2.origHand):
...
def get_best_hand(hand: str):
    ...
    hand_arr = [c for c in hand]
    best_hand = "".join(hand_arr)
    for combo in k:
        for i, c in zip(js, combo):
            hand_arr[i] = c
        curr_hand = "".join(hand_arr)
        if cmp_hand_with_jokers(HandWithJoker(curr_hand, hand), HandWithJoker(best_hand, hand)) > 0:
            best_hand = curr_hand
...
def part_b(filename):
    ...
    with open(filename) as f:
        ...
        w = 0
        sortedHands = sorted(hands.keys(), key=cmp_to_key(cmp_hand_with_jokers))
        for i, h in enumerate(sortedHands):
            w += (i+1) * hands[h]
        print(w)
